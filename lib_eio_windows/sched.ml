...
type Fiber_context.scheduler_tag += Windows_scheduler of t

let t_of_suspended (k : _ Suspended.t) =
  match Fiber_context.scheduler_tag k.fiber with
  | Windows_scheduler t -> t
  | _ -> invalid_arg "expected windows scheduler tag!"
...
let with_sched fn =
  ...
  let rec t =
    { run_q; poll; fd_map; eventfd; eventfd_r;
      active_ops = 0; need_wakeup = Atomic.make false; sleep_q; thread_pool;
      scheduler_tag = Windows_scheduler t } in
...
let run ~extra_effects t main x =
  ...
  and with_nested_sched : type a. t:t -> (unit -> a) -> a =
    fun ~t:parent fn ->
    let rec t = { parent with scheduler_tag = Windows_scheduler t; run_q = Lf_queue.create () } in
